...
Then we injcet the property through prototype polution and check if everything is working correctly. As we can see, we successfully injected the property `ppp` into the node.js `__proto__`.
